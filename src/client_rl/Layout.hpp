...
// Resolution-independent layout primitives, shared conceptually with
// src/client/Layout.hpp (the Win32 client) — this copy has zero platform
// dependency (not even raylib) so it stays reusable regardless of rendering
// backend. Convert a Rect to raylib's `Rectangle` inline at the call site
// (`Rectangle{(float)r.left, (float)r.top, (float)r.width(), (float)r.height()}`)
// rather than baking that conversion in here.
//
...
// edge, shrinking the remainder each time. Every screen derives its widget
// rects this way from the *current* client rect, so paint and hit-testing
// always agree and resizing just recomputes the same cuts.
...
// sizing and font point sizes track the same factor.
struct UiScale {
    float layout{1.0f};
    int dpi{96};
